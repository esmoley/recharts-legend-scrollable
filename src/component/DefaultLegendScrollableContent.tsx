 ...
 */
import React, { PureComponent, ReactNode, MouseEvent, ReactText, ReactElement, CSSProperties } from 'react'
...
interface InternalProps {
  ...
  chartWidth?: number
  chartHeight?: number
  scrollbarStyle?: 'none' | 'default'
}

export declare type Props = InternalProps & PresentationAttributesAdaptChildEvent<any, ReactElement>

export class DefaultLegendScrollableContent extends PureComponent<Props> {
  ...
  render() {
    const { payload, layout, align, chartHeight, contentMaxHeight, scrollbarStyle } = this.props
    ...
    let className = 'recharts-default-legend recharts-scrollable-legend'
    if (scrollbarStyle !== 'none') {
      className = `${className} recharts-scrollable-legend-${scrollbarStyle}`
    }
    return (
      <ul className={className} style={finalStyle}>
        {this.renderItems()}
      </ul>
    )
  }
